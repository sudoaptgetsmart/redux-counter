...
import {ON_ADD, ON_MUL, ON_SUB, ON_DIV, ON_SET_INPUT1, ON_SET_INPUT2} from "../modules/counter";
...
test('should show an input with a placeholder of "Some numbers" and type of number that dispatches ON_SET_INPUT1',
    () => {
        const dispatch = jest.fn()
        render(<Calculator _useDispatch={() => dispatch} _useSelector={() => {
        }}/>)
...
test('should show an input with a placeholder of "Some more numbers" and type of number that dispatches ON_SET_INPUT2',
    () => {
        const dispatch = jest.fn()
        render(<Calculator _useDispatch={() => dispatch} _useSelector={() => {}}/>)

        const input = screen.getByPlaceholderText('Some more numbers');
        expect(input).toHaveAttribute('type', 'number')
